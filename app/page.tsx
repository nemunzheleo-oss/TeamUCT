...
const invoices = [
  { id: 'INV-10482', vendor: 'Northstar Scientific', date: '18 Sep 2025', amount: 'R12,480.00', status: 'Needs review', issue: 'Price variance', tone: 'amber' },
  { id: 'INV-10477', vendor: 'Campus Office Co.', date: '17 Sep 2025', amount: 'R2,160.40', status: 'Awaiting GRV', issue: 'Goods not receipted', tone: 'slate' },
  { id: 'INV-10471', vendor: 'Brightline Facilities', date: '16 Sep 2025', amount: 'R8,900.00', status: 'Exception', issue: 'Missing PO number', tone: 'amber' },
  { id: 'INV-10468', vendor: 'Lumen IT Services', date: '15 Sep 2025', amount: 'R4,250.00', status: 'Vendor mismatch', issue: 'Bank details differ', tone: 'red' },
]
...
function FinanceView() {
  ...
  return <>{!unlocked && <TwoFactor onUnlock={() => setUnlocked(true)} />}<header className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Finance manager</p><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Procurement control centre</h1><p className="mt-2 text-sm text-muted-foreground">Review exceptions, clear bottlenecks, and keep payments on track.</p></div><div className="flex items-center gap-2"><Button variant="outline" size="icon" aria-label="Notifications"><Bell className="size-4" /></Button><Button className="gap-2"><Sparkles className="size-4" /> Run smart review</Button></div></header>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[['Pending invoices','24','Needs attention','amber'],['Due this week','R48,920','Across 9 invoices','slate'],['Avg. approval time','2.4 days','18% faster than last month','green'],['Exception rate','8.3%','4 invoices flagged','red']].map(([label,value,detail,tone]) => <div key={label} className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="flex items-center justify-between"><p className="text-xs font-medium text-muted-foreground">{label}</p><span className={`size-2 rounded-full ${tone === 'amber' ? 'bg-amber-500' : tone === 'green' ? 'bg-emerald-500' : tone === 'red' ? 'bg-red-500' : 'bg-slate-300'}`} /></div><p className="mt-3 text-2xl font-semibold tracking-tight">{value}</p><p className={`mt-1 text-xs ${tone === 'green' ? 'text-emerald-700' : tone === 'red' ? 'text-red-700' : 'text-muted-foreground'}`}>{detail}</p></div>)}</div>
